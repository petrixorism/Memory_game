...
import uz.ravshanbaxranov.memorygame.animation.GameAnimation
import uz.ravshanbaxranov.memorygame.data.LeaderEntity
import uz.ravshanbaxranov.memorygame.databinding.FragmentPlaygroundBinding
import uz.ravshanbaxranov.memorygame.ui.viewmodel.PlaygroundViewModel
...
class PlaygroundFragment : Fragment(R.layout.fragment_playground) {
    ...
    private val animator by lazy { GameAnimation(lifecycleScope) }
...
//
        ...
        homeBtn.setOnClickListener {
            ...
            if (isTopUser && nameEt.text.isNotBlank()) {
        ...
        retryBtn.setOnClickListener {
            if (nameEt.text.toString().isNotBlank()) {
                if (isTopUser) {
                    viewModel.insertUserToLeaderboard(
                        LeaderEntity(
                            name = nameEt.text.toString(),
                ...
                binding.chronometer.start()
                setImages(1000L)
            } else Toast.makeText(requireContext(), "Enter your name!", Toast.LENGTH_SHORT).show()
        }
        dialog.show()
